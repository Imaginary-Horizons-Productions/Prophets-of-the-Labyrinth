...
const urgentBloodAegis = new GearTemplate("Urgent Blood Aegis",
	...
	.setEffect(urgentBloodAegisEffect, { type: "single", team: "foe" })
	.setPactCost([5, "Pay (@{pactCost} x your level) HP"])
	.setScalings({
		protection: protectionScalingGenerator(125),
		critBonus: 2,
...
/** @type {typeof urgentBloodAegis.effect} */
function urgentBloodAegisEffect([target], user, adventure) {
	const { essence, pactCost, scalings: { protection, critBonus } } = urgentBloodAegis;
	const paymentSentence = payHP(user, user.level * pactCost[0], adventure);
	if (adventure.lives < 1) {
		return [paymentSentence];
	}
