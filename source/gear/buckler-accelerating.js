...
module.exports = new GearTemplate("Accelerating Buckler",
	...
	(targets, user, adventure) => {
		...
		return [joinAsStatement(false, [user, ...targets].map(combatant => combatant.name), "gains", "gain", "protection."), ...generateModifierResultLines(addModifier([user], swiftness))];
	}
).setTargetingTags({ type: "single", team: "ally" })
	.setSidegrades("Devoted Buckler", "Guarding Buckler")
	.setModifiers({ name: "Swiftness", stacks: 6 })
	.setCooldown(1)
	.setProtection(75);
