...
module.exports = new GearTemplate("Accelerating Midas Staff",
	...
	([target], user, adventure) => {
		const { essence, modifiers: [curse, swiftness], bonus } = module.exports;
		...
		const receipts = addModifier([target], pendingCurse).concat(addModifier([user], swiftness));
		return generateModifierResultLines(combineModifierReceipts(receipts));
	}
).setTargetingTags({ type: "single", team: "any" })
	.setSidegrades("Discounted Midas Staff", "Soothing Midas Staff")
	.setModifiers({ name: "Curse of Midas", stacks: 2 }, { name: "Swiftness", stacks: 1 })
