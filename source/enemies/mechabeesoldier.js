...
module.exports = new EnemyTemplate("Mechabee Soldier",
	...
	effect: (targets, user, adventure) => {
		...
		changeStagger(targets, user, ESSENCE_MATCH_STAGGER_FOE);
		return dealDamage(targets, user, damage, false, user.essence, adventure).concat(generateModifierResultLines(addModifier(targets, { name: "Paralysis", stacks: user.crit ? 5 : 3 })));
	},
	selector: selectRandomFoe,
	next: "Self-Destruct"
}).addAction({
